...
if not MONGO_URI:
    sys.exit("❌ .env must have MONGO_URI")
...
try:
    ...
    sys.exit("❌ Failed to connect to MongoDB. Check MONGO_URI")
...
logger_data_collection = db["logger_data"]

# Create indexes for better performance
try:
    users_collection.create_index("user_id", unique=True)
    sessions_collection.create_index("user_id", unique=True)
    logger_data_collection.create_index("user_id")
...
ADS_WORKERS: Dict[int, asyncio.Task] = {}
...
STEP_CT_NAME = "CT_NAME"  # Custom Topics - ask campaign name
STEP_CT_POST_LINK = "CT_POST_LINK"  # Custom Topics - ask post link
STEP_CT_FALLBACK = "CT_FALLBACK"  # Custom Topics - ask fallback
STEP_CT_SELECT_TOPICS = "CT_SELECT_TOPICS"  # Custom Topics - select topics
STEP_SET_POST_LINK = "SET_POST_LINK"  # Persistent - set post link
STEP_SET_FALLBACK = "SET_FALLBACK"  # Persistent - set fallback
STEP_SET_ROUND_DELAY = "SET_ROUND_DELAY"  # Persistent - set round delay
STEP_SET_MSG_DELAY = "SET_MSG_DELAY"  # Persistent - set message delay

# Morefeatures now integrated directly into main.py - no external file needed

# ---------- Helpers ----------
# Temporary sessions directory (for Telethon runtime - created on demand)
SESSIONS_DIR = Path("./sessions")

# Helper functions for session management
def upload_session_to_mongodb(user_id: int, session_path: str):
    """Upload Telethon session file to MongoDB as binary"""
    try:
        ...
        sessions_collection.update_one(
            ...
            {"$set": {
                "user_id": user_id,
                "session_data": base64.b64encode(session_data).decode('utf-8'),
                "session_path": session_path,
                "uploaded_at": time.time()
            }},
            upsert=True
        )
        print(f"✅ Session uploaded to MongoDB for user {user_id}")
    except Exception as e:
        print(f"⚠️ Failed to upload session for user {user_id}: {e}")

def download_session_from_mongodb(user_id: int, session_path: str) -> bool:
    """Download Telethon session file from MongoDB"""
    try:
        session_doc = sessions_collection.find_one({"user_id": user_id})
        if not session_doc or "session_data" not in session_doc:
            return False
        
        # Decode and write session file
        session_data = base64.b64decode(session_doc["session_data"])
        session_file = Path(session_path)
        ...
        print(f"✅ Session downloaded from MongoDB for user {user_id}")
        return True
    except Exception as e:
        print(f"⚠️ Failed to download session for user {user_id}: {e}")
        return False

def load_user(user_id: int, force: bool = False) -> Dict[str, Any]:
    """Load user data from MongoDB with local cache"""
    # force=True ensures /start and callbacks immediately reflect changes
    ...
    try:
        ...
        print(f"⚠️ MongoDB load error for user {user_id}: {e}")
        USERS[user_id] = {"user_id": user_id}
    
    U = USERS[user_id]
    U.setdefault("step", STEP_NONE)
    U.setdefault("last_msg", {"chat_id": None, "message_id": None, "is_photo": False})
    U.setdefault("custom_topic_campaigns", [])  # List of custom topic campaigns
    U.setdefault("ct_temp", {})  # Temporary storage for creating custom topic campaign
    
    # Persistent settings
    U.setdefault("persistent_settings", {
        "forward_mode": None,  # "saved_message" or "post_link"
        "post_link": None,
        "saved_from_peer": None,
        "saved_msg_id": None,
        "fallback_message": None,
        "group_mode": None,  # "all_groups", "selected_groups", or "custom_topics"
        "selected_groups": [],  # For selected_groups mode
        "round_delay": 3600,  # Default 1 hour
        "message_delay": 15   # Default 15 seconds
    })
    ...
    U.setdefault("settings", {
        "forward_mode": None,  # "saved_message" or "post_link"
        "post_link": None,
        "post_link_parsed": None,  # {"from_peer": ..., "msg_id": ...}
        "fallback_message": None,
        "group_mode": None,  # "all_joined", "selected", "custom_topics"
        "selected_groups": [],  # For "selected" mode
        "active_campaign_idx": None,  # For "custom_topics" mode
        "round_delay": 60,  # Default 60 seconds
        "message_delay": 180  # Default 180 seconds (SEND_GAP_MAX)
    })
    U.setdefault("login", {
        "api_id": None,
        "api_hash": None,
    ...
    # Download session from MongoDB if it exists locally or in cloud
    session_path = U["session_base"]
    session_file = Path(session_path)
    
    if not session_file.exists():
        # Try to download from MongoDB
        download_session_from_mongodb(user_id, session_path)
...
def save_user(user_id: int):
    ...
    try:
        ...
        user_data["user_id"] = user_id  # Ensure user_id is in document
        ...
        print(f"⚠️ MongoDB save error for user {user_id}: {e}")
...
def custom_topics_main_kb() -> InlineKeyboardMarkup:
    """Main menu for custom topics campaigns"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Create New Campaign", callback_data="ct:create")],
        [InlineKeyboardButton("📋 View Campaigns", callback_data="ct:list")],
        [InlineKeyboardButton("🔙 Back to Menu", callback_data="back_to_main")]
    ])

def custom_topics_list_kb(user_id: int) -> InlineKeyboardMarkup:
    """Show list of custom topic campaigns"""
    u = load_user(user_id)
    campaigns = u.get("custom_topic_campaigns", [])
    
    buttons = []
    for idx, campaign in enumerate(campaigns):
        name = campaign.get("name", f"Campaign {idx+1}")
        topic_count = len(campaign.get("topics", []))
        buttons.append([InlineKeyboardButton(
            f"📁 {name} ({topic_count} topics)", 
            callback_data=f"ct:view:{idx}"
        )])
    
    buttons.append([InlineKeyboardButton("🔙 Back", callback_data="custom_topics")])
    return InlineKeyboardMarkup(buttons)

def custom_topic_campaign_kb(campaign_idx: int) -> InlineKeyboardMarkup:
    """Show options for a specific campaign"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("▶️ Start Campaign", callback_data=f"ct:start:{campaign_idx}")],
        [InlineKeyboardButton("✏️ Edit", callback_data=f"ct:edit:{campaign_idx}")],
        [InlineKeyboardButton("🗑️ Delete", callback_data=f"ct:delete:{campaign_idx}")],
        [InlineKeyboardButton("🔙 Back", callback_data="ct:list")]
    ])

def new_main_menu_kb(user_id: int) -> InlineKeyboardMarkup:
    """New main menu with persistent settings"""
    u = load_user(user_id)
    ps = u.get("persistent_settings", {})
    
    # Get current settings with indicators
    forward_mode = ps.get("forward_mode")
    if forward_mode == "saved_message":
        forward_text = "📨 Saved ✅"
    elif forward_mode == "post_link":
        forward_text = "🔗 Post ✅"
    else:
        forward_text = "❌"
    
    group_mode = ps.get("group_mode")
    if group_mode == "all_groups":
        group_text = "📚 All ✅"
    elif group_mode == "selected_groups":
        group_text = "🎯 Selected ✅"
    elif group_mode == "custom_topics":
        group_text = "🎯 Topics ✅"
    else:
        group_text = "❌"
    
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 Start Ads", callback_data="start_ads_new"),
         InlineKeyboardButton("🛑 Stop Ads", callback_data="stop_ads")],
        [InlineKeyboardButton(f"📤 Forward Mode: {forward_text}", callback_data="menu_forward_mode")],
        [InlineKeyboardButton(f"👥 Group Mode: {group_text}", callback_data="menu_group_mode")],
        [InlineKeyboardButton("⏱️ Interval Management", callback_data="menu_intervals")],
        [InlineKeyboardButton("🎯 Custom Topics", callback_data="custom_topics")],
...
def forward_mode_menu_kb(user_id: int) -> InlineKeyboardMarkup:
    """Forward mode submenu"""
    u = load_user(user_id)
    ps = u.get("persistent_settings", {})
    forward_mode = ps.get("forward_mode")
    post_link = ps.get("post_link")
    fallback = ps.get("fallback_message")
    
    saved_check = "✅" if forward_mode == "saved_message" else "☐"
    post_check = "✅" if forward_mode == "post_link" else "☐"
    
    buttons = [
        [InlineKeyboardButton(f"{saved_check} Saved Message", callback_data="fwd:saved")],
        [InlineKeyboardButton(f"{post_check} Post Link", callback_data="fwd:post")],
    ]
    
    if forward_mode == "post_link":
        if post_link:
            buttons.append([InlineKeyboardButton("🔗 Set Post Link ✅", callback_data="fwd:set_link")])
            buttons.append([InlineKeyboardButton("🗑️ Clear Post Link", callback_data="fwd:clear_link")])
        else:
            buttons.append([InlineKeyboardButton("🔗 Set Post Link", callback_data="fwd:set_link")])
    
    if fallback:
        buttons.append([InlineKeyboardButton("💬 Set Fallback ✅", callback_data="fwd:set_fallback")])
        buttons.append([InlineKeyboardButton("🗑️ Clear Fallback", callback_data="fwd:clear_fallback")])
    else:
        buttons.append([InlineKeyboardButton("💬 Set Fallback", callback_data="fwd:set_fallback")])
    
    buttons.append([InlineKeyboardButton("🔙 Back", callback_data="back_to_main_new")])
    return InlineKeyboardMarkup(buttons)

def group_mode_menu_kb(user_id: int) -> InlineKeyboardMarkup:
    """Group mode submenu"""
    u = load_user(user_id)
    ps = u.get("persistent_settings", {})
    group_mode = ps.get("group_mode")
    
    all_check = "✅" if group_mode == "all_groups" else "☐"
    selected_check = "✅" if group_mode == "selected_groups" else "☐"
    custom_check = "✅" if group_mode == "custom_topics" else "☐"
    
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{all_check} All Joined Groups", callback_data="gmode:all")],
        [InlineKeyboardButton(f"{selected_check} Selected Groups", callback_data="gmode:selected")],
        [InlineKeyboardButton(f"{custom_check} Custom Topics", callback_data="gmode:custom")],
        [InlineKeyboardButton("🔙 Back", callback_data="back_to_main_new")]
    ])

def interval_menu_kb(user_id: int) -> InlineKeyboardMarkup:
    """Interval management submenu"""
    u = load_user(user_id)
    ps = u.get("persistent_settings", {})
    group_mode = ps.get("group_mode")
    round_delay = ps.get("round_delay", 3600)
    msg_delay = ps.get("message_delay", 15)
    
    # If Custom Topics mode is selected, show hardcoded values (not clickable)
    if group_mode == "custom_topics":
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("⏱️ Round Delay: 3600s (Fixed for Custom Topics)", callback_data="noop")],
            [InlineKeyboardButton("⏳ Message Delay: 10s (Fixed for Custom Topics)", callback_data="noop")],
            [InlineKeyboardButton("🔙 Back", callback_data="back_to_main_new")]
        ])
    else:
        # For All Groups or Selected Groups - user can set freely
        return InlineKeyboardMarkup([
            [InlineKeyboardButton(f"⏱️ Round Delay: {round_delay}s (Click to change)", callback_data="int:round")],
            [InlineKeyboardButton(f"⏳ Message Delay: {msg_delay}s (Click to change)", callback_data="int:msg")],
            [InlineKeyboardButton("🔙 Back", callback_data="back_to_main_new")]
        ])
...
def has_started_logger(uid: int) -> bool:
    """Check if user has started the logger bot (from MongoDB)"""
    try:
        logger_doc = logger_data_collection.find_one({"user_id": uid})
        if logger_doc:
            return logger_doc.get("started", False)
        return False
    except Exception:
        return False

def save_logger_started(uid: int):
    """Save logger bot started status to MongoDB"""
    try:
        logger_data_collection.update_one(
            {"user_id": uid},
            {"$set": {"user_id": uid, "started": True, "started_at": time.time()}},
            upsert=True
        )
    except Exception as e:
        print(f"⚠️ Logger data save error: {e}")
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show new main menu directly
    welcome_msg = (
        f"👋 Welcome, {first}!\n\n"
        f"🎯 Configure your settings and start advertising!\n\n"
        f"Status:\n"
        f"📤 Forward Mode: Not set\n"
        f"👥 Group Mode: Not set\n"
        f"⏱️ Intervals: 3600s / 15s\n"
    )
    
    try:
        sent = await context.bot.send_photo(chat_id=chat_id, photo=BANNER_URL, caption=welcome_msg, reply_markup=new_main_menu_kb(user_id))
        set_last_msg(u, chat_id, sent.message_id, True)
    except Exception:
        sent = await context.bot.send_message(chat_id=chat_id, text=welcome_msg, reply_markup=new_main_menu_kb(user_id))
        set_last_msg(u, chat_id, sent.message_id, False)
    save_user(user_id)
...
async def try_sign_in_with_code(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Upload session to MongoDB
        session_path = u.get("session_base")
        if session_path:
            upload_session_to_mongodb(user_id, session_path)
        
        await delete_otp_message(user_id, context)
        await send_new_banner_text(user_id, context, "✅ Login successful!\n\nWelcome to the main menu.", new_main_menu_kb(user_id))
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "refresh_status":
        ...
        if allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PERSONAL_WELCOME(first), new_main_menu_kb(user_id))
        else:
    ...
    if data == "check_logger":
        ...
        if has_started_logger(user_id):
            ...
            # Return to main menu
            await edit_caption_keep_banner(user_id, context, MAIN_MENU_TEXT, new_main_menu_kb(user_id))
        else:
    ...
    if data == "custom_topics":
        ...
        await edit_caption_keep_banner(
            user_id, 
            context, 
            "🎯 Custom Topics Campaigns\n\nCreate topic-specific campaigns with custom post links for each!",
            custom_topics_main_kb()
        )
        return

    # Custom Topics - Create new campaign
    if data == "ct:create":
        await q.answer()
        u["ct_temp"] = {}
        u["step"] = STEP_CT_NAME
        ...
        await edit_caption_keep_banner(
            user_id,
            context,
            "📝 Create New Campaign\n\nEnter a name for this campaign (e.g., 'Telegram', 'Crypto', 'Gaming'):",
            None
        )
        return

    # Custom Topics - List campaigns
    if data == "ct:list":
        await q.answer()
        campaigns = u.get("custom_topic_campaigns", [])
        if not campaigns:
            await edit_caption_keep_banner(
                user_id,
                context,
                "📋 No campaigns yet!\n\nCreate your first custom topic campaign.",
                custom_topics_main_kb()
            )
        else:
            await edit_caption_keep_banner(
                user_id,
                context,
                f"📋 Your Campaigns ({len(campaigns)})\n\nSelect a campaign to view details:",
                custom_topics_list_kb(user_id)
            )
        return

    # Custom Topics - View specific campaign
    if data.startswith("ct:view:"):
        await q.answer()
        ...
        post_link = campaign.get("post_link", "N/A")
        fallback = campaign.get("fallback_message", "N/A")
        topics = campaign.get("topics", [])
        
        # Regular view from Custom Topics menu
        msg = (
            f"📁 Campaign: {name}\n"
        ...
        await edit_caption_keep_banner(user_id, context, msg, custom_topic_campaign_kb(idx))
        return

    # Custom Topics - Delete campaign
    if data.startswith("ct:delete:"):
        ...
        if idx < len(campaigns):
            del campaigns[idx]
            save_user(user_id)
            await q.answer("Campaign deleted!", show_alert=True)
            await edit_caption_keep_banner(
                user_id,
                context,
    ...
    # Custom Topics - Start campaign
    if data.startswith("ct:start:"):
        await q.answer()
        idx = int(data.split(":")[2])
        campaigns = u.get("custom_topic_campaigns", [])
        if idx >= len(campaigns):
        ...
        # Setup ad_setup with campaign data (no delays asked)
        u["ad_setup"] = {
            "message_source": "Custom Topics Campaign",
            "message_text": None,
            "media_path": None,
            "media_type": None,
            "saved_msg_id": campaign["saved_msg_id"],
            "saved_from_peer": campaign["saved_from_peer"],
            "saved_as_copy": False,  # Use forwarding with fallback
            "post_link": campaign["post_link"],
            "fallback_message": campaign["fallback_message"],
            "targets": campaign["topics"],
            "round_delay": campaign["round_delay"],
            "send_gap_max": campaign["send_gap_max"]
        }
        save_user(user_id)
        
        # Start the campaign immediately
        if user_id in ADS_WORKERS and ADS_WORKERS[user_id] and not ADS_WORKERS[user_id].done():
            await q.answer("Ads are already running. Stop them first.", show_alert=True)
            return
        
        # Trigger ads_worker
        ADS_WORKERS[user_id] = asyncio.create_task(ads_worker(user_id, context))
        ...
        await edit_caption_keep_banner(
            ...
            f"🚀 Campaign Started!\n\n"
            f"📁 {campaign['name']}\n"
            f"📌 Sending to {len(campaign['topics'])} topics\n"
            f"⏱️ Round: 1 hour | Gap: 10s\n\n"
            f"Check logger bot for updates!",
            InlineKeyboardMarkup([[InlineKeyboardButton("🛑 Stop", callback_data="stop_ads")]])
        )
        return
    ...
    if data == "back_to_main":
        ...
        await edit_caption_keep_banner(
            user_id,
            context,
            f"👋 Welcome back, {first}!\n\n📋 Main Menu\nConfigure your ad settings and start campaigns:",
            new_main_menu_kb(user_id)
        )
        return

    # Back to new main menu
    if data == "back_to_main_new":
        await q.answer()
        ps = u.get("persistent_settings", {})
        
        # Get readable names
        forward_text = "📨 Saved Message" if ps.get('forward_mode') == "saved_message" else "🔗 Post Link" if ps.get('forward_mode') == "post_link" else "❌ Not set"
        
        group_mode = ps.get('group_mode')
        if group_mode == "all_groups":
            group_text = "📚 All Joined Groups"
        elif group_mode == "selected_groups":
            selected_count = len(ps.get('selected_groups', []))
            group_text = f"🎯 Selected Groups ({selected_count})"
        elif group_mode == "custom_topics":
            group_text = "🎯 Custom Topics (Fixed delays)"
        else:
            group_text = "❌ Not set"
        
        # Show intervals based on mode
        if group_mode == "custom_topics":
            interval_text = "⏱️ 3600s / 10s (Fixed for Custom Topics)"
        else:
            interval_text = f"⏱️ {ps.get('round_delay', 3600)}s / {ps.get('message_delay', 15)}s"
        
        post_link_status = "✅" if ps.get('post_link') else "❌"
        fallback_status = "✅" if ps.get('fallback_message') else "❌"
        
        status_msg = (
            f"👋 Welcome, {first}!\n\n"
            f"📋 Current Configuration:\n\n"
            f"📤 Forward Mode: {forward_text}\n"
            f"🔗 Post Link: {post_link_status}\n"
            f"💬 Fallback: {fallback_status}\n"
            f"👥 Group Mode: {group_text}\n"
            f"{interval_text}\n"
        )
        await edit_caption_keep_banner(user_id, context, status_msg, new_main_menu_kb(user_id))
        return

    # === FORWARD MODE MENU ===
    if data == "menu_forward_mode":
        await q.answer()
        await edit_caption_keep_banner(
            user_id,
            context,
            "📤 Forward Mode Settings\n\nSelect how you want to send messages:",
            forward_mode_menu_kb(user_id)
        )
        return

    if data == "fwd:saved":
        await q.answer("Saved Message mode selected!")
        ps = u.get("persistent_settings", {})
        ps["forward_mode"] = "saved_message"
        save_user(user_id)
        # Update live
        await edit_caption_keep_banner(user_id, context, "✅ Saved Message mode active!", forward_mode_menu_kb(user_id))
        return

    if data == "fwd:post":
        await q.answer("Post Link mode selected!")
        ps = u.get("persistent_settings", {})
        ps["forward_mode"] = "post_link"
        save_user(user_id)
        # Update live
        await edit_caption_keep_banner(user_id, context, "✅ Post Link mode active!\n\nNow set your post link below:", forward_mode_menu_kb(user_id))
        return

    if data == "fwd:set_link":
        await q.answer()
        u["step"] = STEP_SET_POST_LINK
        save_user(user_id)
        await edit_caption_keep_banner(user_id, context, "🔗 Send me the post link (e.g., https://t.me/username/123):", None)
        return

    if data == "fwd:clear_link":
        await q.answer("Post link cleared!")
        ps = u.get("persistent_settings", {})
        ps["post_link"] = None
        ps["saved_from_peer"] = None
        ps["saved_msg_id"] = None
        save_user(user_id)
        # Update live
        await edit_caption_keep_banner(user_id, context, "🗑️ Post link cleared!", forward_mode_menu_kb(user_id))
        return

    if data == "fwd:set_fallback":
        await q.answer()
        u["step"] = STEP_SET_FALLBACK
        save_user(user_id)
        await edit_caption_keep_banner(user_id, context, "💬 Send me the fallback message:", None)
        return

    if data == "fwd:clear_fallback":
        await q.answer("Fallback cleared!")
        ps = u.get("persistent_settings", {})
        ps["fallback_message"] = None
        save_user(user_id)
        # Update live
        await edit_caption_keep_banner(user_id, context, "🗑️ Fallback cleared!", forward_mode_menu_kb(user_id))
        return

    # === GROUP MODE MENU ===
    if data == "menu_group_mode":
        await q.answer()
        await edit_caption_keep_banner(user_id, context, "👥 Group Mode Settings\n\nChoose target groups:", group_mode_menu_kb(user_id))
        return

    if data == "gmode:all":
        await q.answer("All Groups mode selected!")
        ps = u.get("persistent_settings", {})
        ps["group_mode"] = "all_groups"
        save_user(user_id)
        # Update live
        await edit_caption_keep_banner(user_id, context, "✅ All Joined Groups mode active!\n\nIntervals are now editable.", group_mode_menu_kb(user_id))
        return

    if data == "gmode:selected":
        await q.answer()
        ps = u.get("persistent_settings", {})
        ps["group_mode"] = "selected_groups"
        save_user(user_id)
        
        # Show group picker
        ok = await collect_user_groups(user_id)
        if not ok:
            await q.answer("Failed to load groups!", show_alert=True)
            return
        
        await show_group_picker(user_id, context)
        return

    if data == "gmode:custom":
        await q.answer("Custom Topics mode selected!")
        ps = u.get("persistent_settings", {})
        ps["group_mode"] = "custom_topics"
        save_user(user_id)
        # Update live - intervals locked to 3600s/10s
        await edit_caption_keep_banner(user_id, context, "✅ Custom Topics mode active!\n\n⚠️ Intervals locked to 3600s/10s\n\nManage campaigns from Custom Topics menu.", group_mode_menu_kb(user_id))
        return

    # === INTERVAL MENU ===
    if data == "menu_intervals":
        ps = u.get("persistent_settings", {})
        group_mode = ps.get("group_mode")
        ...
        if group_mode == "custom_topics":
            ...
            await edit_caption_keep_banner(
                ...
                "🎯 Custom Topics Mode Active\n\n"
                "Delays are hardcoded:\n"
                "⏰ Round Delay: 3600s (1 hour)\n"
                "⏳ Message Delay: 10s\n\n"
                "These cannot be changed for Custom Topics campaigns.",
                interval_menu_kb(user_id)
            )
        else:
            await q.answer()
            mode_text = "📚 All Groups" if group_mode == "all_groups" else "🎯 Selected Groups" if group_mode == "selected_groups" else "Not Set"
            await edit_caption_keep_banner(
                user_id, 
                context, 
                f"⏱️ Interval Management\n\n"
                f"Mode: {mode_text}\n\n"
                f"Set custom delays for your campaigns:",
                interval_menu_kb(user_id)
            )
        return
    ...
    if data == "int:round":
        await q.answer()
        u["step"] = STEP_SET_ROUND_DELAY
        save_user(user_id)
        await edit_caption_keep_banner(user_id, context, "⏱️ Set Round Delay\n\nEnter delay in seconds (minimum 1s):\n\nExamples:\n• 60 = 1 minute\n• 3600 = 1 hour\n• 7200 = 2 hours", None)
        return

    if data == "int:msg":
        await q.answer()
        u["step"] = STEP_SET_MSG_DELAY
        save_user(user_id)
        await edit_caption_keep_banner(user_id, context, "⏳ Set Message Delay\n\nEnter delay in seconds (minimum 1s):\n\nExamples:\n• 5 = 5 seconds\n• 15 = 15 seconds\n• 30 = 30 seconds", None)
        return

    # === START ADS (NEW) ===
    if data == "start_ads_new":
        ps = u.get("persistent_settings", {})
        ...
        forward_mode = ps.get("forward_mode")
        if not forward_mode:
            errors.append("❌ Forward Mode not set")
        elif forward_mode == "post_link" and not ps.get("post_link"):
            errors.append("❌ Post Link not set")
        
        # Check group mode
        group_mode = ps.get("group_mode")
        if not group_mode:
            errors.append("❌ Group Mode not set")
        elif group_mode == "selected_groups" and not ps.get("selected_groups"):
            errors.append("❌ No groups selected")
        
        # Check fallback
        if not ps.get("fallback_message"):
            errors.append("⚠️ Fallback message not set (recommended)")
        
        if errors:
            await q.answer("Configuration incomplete!", show_alert=True)
            await edit_caption_keep_banner(
                user_id,
                context,
        ...
        # 2. Read from persistent_settings
        # 3. Determine which mode (all/selected/custom topics)
        
        if group_mode == "custom_topics":
            # For custom topics, show campaign list
            campaigns = u.get("custom_topic_campaigns", [])
            if not campaigns:
                await q.answer("No campaigns found!", show_alert=True)
                return
        ...
        # For all_groups or selected_groups modes
        # Check if already running
        if user_id in ADS_WORKERS and ADS_WORKERS[user_id] and not ADS_WORKERS[user_id].done():
            await q.answer("Ads already running!", show_alert=True)
            return
        
        # 4. For custom topics: use fixed 3600s/10s delays
        # 5. For all/selected: use user-configured delays
        round_delay = ps.get("round_delay", 3600)
        message_delay = ps.get("message_delay", 15)
        
        # 6. Build ad_setup and start ads_worker
        ...
        if group_mode == "all_groups":
            ...
            # Filter regular groups only
            all_groups = u["group_picker"]["groups"]
            regular_groups = [g for g in all_groups if g.get("group_type") == "group"]
            targets = [{"display_id": g["display_id"]} for g in regular_groups]
        else:  # selected_groups
            targets = [{"display_id": gid} for gid in ps.get("selected_groups", [])]
        
        if not targets:
            await q.answer("No targets found!", show_alert=True)
            return
        
        # Build ad_setup
        if forward_mode == "saved_message":
            # Use latest saved message from "me"
            ...
            if not msgs or not msgs[0]:
                await q.answer("No saved messages found!", show_alert=True)
                return
            
            m = msgs[0]
            u["ad_setup"] = {
                "message_source": "Saved message",
                ...
                "saved_as_copy": False,
                "post_link": None,
                "fallback_message": ps.get("fallback_message"),
                "targets": targets,
                "round_delay": round_delay,
                "send_gap_max": message_delay
            }
        else:  # post_link
            u["ad_setup"] = {
                ...
                "media_path": None,
                "media_type": None,
                "saved_msg_id": ps.get("saved_msg_id"),
                "saved_from_peer": ps.get("saved_from_peer"),
                "saved_as_copy": False,
                "post_link": ps.get("post_link"),
                "fallback_message": ps.get("fallback_message"),
                "targets": targets,
        ...
        await edit_caption_keep_banner(
            ...
            f"⏱️ Round: {round_delay}s | Gap: {message_delay}s\n\n"
            f"Check logger bot for updates!",
            new_main_menu_kb(user_id)
        )
        return
    ...
    if data == "picker_continue":
        ...
        # Check if we're in Custom Topics mode
        if u["step"] == STEP_CT_SELECT_TOPICS:
            await q.answer()
            # Save the custom topic campaign
            ct_temp = u["ct_temp"]
            selected_topics = [{"display_id": sid} for sid in sel]
            
            campaign = {
                "name": ct_temp.get("name", "Unnamed"),
                "post_link": ct_temp.get("post_link"),
                "saved_from_peer": ct_temp.get("saved_from_peer"),
                "saved_msg_id": ct_temp.get("saved_msg_id"),
                "fallback_message": ct_temp.get("fallback_message"),
                "topics": selected_topics,
                "round_delay": 3600,  # Fixed 1 hour
                "send_gap_max": 10    # Fixed 10 seconds
            }
            
            u["custom_topic_campaigns"].append(campaign)
            u["ct_temp"] = {}
            u["step"] = STEP_NONE
            save_user(user_id)
            ...
            await edit_caption_keep_banner(
                ...
                f"⏱️ Round Delay: 1 hour\n"
                f"⏳ Send Gap: 10 seconds\n\n"
                f"Ready to start!",
                custom_topics_main_kb()
            )
            return
        
        # Check if we're selecting groups for persistent settings "Selected Groups" mode
        ps = u.get("persistent_settings", {})
        if ps.get("group_mode") == "selected_groups":
            await q.answer()
            # Save selected groups to persistent settings
            ps["selected_groups"] = sel
            save_user(user_id)
            
            await edit_caption_keep_banner(
                user_id,
                context,
                f"✅ Selected Groups Saved!\n\n"
                f"📊 {len(sel)} groups selected\n\n"
                f"Ready to start ads!",
                group_mode_menu_kb(user_id)
            )
            return
    ...
    if data == "stop_ads":
        ...
        await edit_banner_strict(user_id, context, ADS_STOPPED_TEXT, new_main_menu_kb(user_id))
        return
    ...
    if data == "my_details":
        ...
        await q.answer()
        await edit_caption_keep_banner(user_id, context, txt, new_main_menu_kb(user_id))
        return
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("step") == STEP_MF_BC_MESSAGE:
        ...
        async def dl(file_id: str, suffix: str) -> str:
            ...
            local = temp_dir / f"bc_{user_id}_{secrets.token_hex(4)}{suffix}"
    ...
    if u["step"] == STEP_ASK_AD_MESSAGE:
        ...
        async def dl(file_id: str, suffix: str) -> str:
            ...
            temp_dir = Path("./temp")
            temp_dir.mkdir(exist_ok=True)
            local = temp_dir / f"ad_{user_id}_{secrets.token_hex(4)}{suffix}"
            await fobj.download_to_drive(custom_path=str(local))
            return str(local)
        ...
        await edit_caption_keep_banner(user_id, context, "✅ Setup complete!\n\nYou can now start your ads.", new_main_menu_kb(user_id))
        return
    ...
    # Custom Topics - Campaign Name
    if u["step"] == STEP_CT_NAME:
        if not allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PREMIUM_UPSELL(first), buy_premium_kb())
            return
        ...
        if not name:
            await context.bot.send_message(chat_id=chat_id, text="❌ Please enter a campaign name.")
            return

        u["ct_temp"]["name"] = name
        u["step"] = STEP_CT_POST_LINK
        save_user(user_id)

        await edit_caption_keep_banner(
            user_id,
            context,
            f"✅ Campaign Name: {name}\n\n🔗 Now send the post link for this campaign:",
            None
        )
        return

    # Custom Topics - Post Link
    if u["step"] == STEP_CT_POST_LINK:
        if not allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PREMIUM_UPSELL(first), buy_premium_kb())
            return
        ...
        parsed = parse_post_link(link)
        if not parsed:
            await context.bot.send_message(chat_id=chat_id, text="❌ Invalid link. Send a link like https://t.me/username/123")
            return

        from_peer, msg_id = parsed
        u["ct_temp"]["post_link"] = link
        u["ct_temp"]["saved_from_peer"] = from_peer
        u["ct_temp"]["saved_msg_id"] = msg_id
        u["step"] = STEP_CT_FALLBACK
        save_user(user_id)

        await edit_caption_keep_banner(
            user_id,
            context,
            "✅ Post link saved!\n\n💬 Now send a fallback message (used when forwarding is blocked):",
            None
        )
        return

    # Custom Topics - Fallback Message
    if u["step"] == STEP_CT_FALLBACK:
        if not allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PREMIUM_UPSELL(first), buy_premium_kb())
            return

        fallback = (txt or "").strip()
        ...
        if not fallback:
            await context.bot.send_message(chat_id=chat_id, text="❌ Please send a fallback message.")
            return

        u["ct_temp"]["fallback_message"] = fallback
        u["step"] = STEP_CT_SELECT_TOPICS
        save_user(user_id)

        # Collect groups to show topics
        client = get_final_client(user_id)
        if client is None:
            await context.bot.send_message(chat_id=chat_id, text="❌ Please login first.")
            return

        ok = await collect_user_groups(user_id)
        if not ok:
            await context.bot.send_message(chat_id=chat_id, text="❌ Failed to load groups.")
            return

        # Filter to show only topics (same as Selected groups UI)
        all_groups = u["group_picker"]["groups"]
        topics_only = [g for g in all_groups if g.get("group_type") == "topic"]
        u["group_picker"]["groups"] = topics_only
        u["group_picker"]["selected_ids"] = []
        u["group_picker"]["page"] = 0
        u["group_picker"]["search_filter"] = ""  # Enable search
        save_user(user_id)

        # Show group picker UI with search functionality
        await show_group_picker(user_id, context)
        return

    # === SETTINGS MESSAGE HANDLERS ===
    
    # Settings - Set Post Link
    if u["step"] == STEP_SET_POST_LINK:
        if not allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PREMIUM_UPSELL(first), buy_premium_kb())
            return
        ...
        ps = u.get("persistent_settings", {})
        ps["post_link"] = link
        ps["saved_from_peer"] = from_peer
        ps["saved_msg_id"] = msg_id
        u["step"] = STEP_NONE
        save_user(user_id)
        ...
        await edit_caption_keep_banner(
            user_id,
            context,
            f"✅ Post link saved!\n\n🔗 {link}\n\nYou can now start ads with this link.",
            forward_mode_menu_kb(user_id)
        )
        return

    # Settings - Set Fallback Message
    if u["step"] == STEP_SET_FALLBACK:
        if not allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PREMIUM_UPSELL(first), buy_premium_kb())
            return
        ...
        await edit_caption_keep_banner(
            user_id,
            context,
            f"✅ Fallback message saved!\n\n💬 {fallback[:100]}...\n\nThis will be used when forwarding is blocked.",
            forward_mode_menu_kb(user_id)
        )
        return

    # Settings - Set Round Delay
    if u["step"] == STEP_SET_ROUND_DELAY:
        if not allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PREMIUM_UPSELL(first), buy_premium_kb())
            return

        delay_str = (txt or "").strip()
        ...
        if not delay_str.isdigit():
            await context.bot.send_message(chat_id=chat_id, text="❌ Please send a valid number.")
            return

        delay = int(delay_str)
        if delay < 1:
            await context.bot.send_message(chat_id=chat_id, text="❌ Minimum round delay is 1 second. You can set any value from 1s upwards.")
            return

        ps = u.get("persistent_settings", {})
        ps["round_delay"] = delay
        u["step"] = STEP_NONE
        save_user(user_id)
        ...
        await edit_caption_keep_banner(
            user_id,
            context,
            f"✅ Round delay set to {delay}s!\n\nThis applies to All Groups and Selected Groups modes.",
            interval_menu_kb(user_id)
        )
        return

    # Settings - Set Message Delay
    if u["step"] == STEP_SET_MSG_DELAY:
        if not allowed_to_use(user_id, u):
            await edit_caption_keep_banner(user_id, context, PREMIUM_UPSELL(first), buy_premium_kb())
            return
        ...
        delay = int(delay_str)
        if delay < 1:
            await context.bot.send_message(chat_id=chat_id, text="❌ Message delay must be at least 1 second.")
            return

        ps = u.get("persistent_settings", {})
        ps["message_delay"] = delay
        u["step"] = STEP_NONE
        save_user(user_id)
        ...
        await edit_caption_keep_banner(
            user_id,
            context,
            f"✅ Message delay set to {delay}s!\n\nThis applies to All Groups and Selected Groups modes.",
            interval_menu_kb(user_id)
        )
        return
        ...
        await edit_caption_keep_banner(
            user_id,
            context,
            f"✅ Message delay set to {delay} seconds!\n\nThis applies to All Joined Groups and Selected Groups modes.",
            interval_kb(user_id)
        )
        return
...
async def start_ads_loop(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t and not t.done():
        await edit_banner_strict(user_id, context, "Ads already started.", new_main_menu_kb(user_id))
        return
    a = u["ad_setup"]
    if not a["setup"] or (not a["saved_msg_id"] and not a["message_text"] and not a["media_path"]) or not a["targets"]:
        await edit_banner_strict(user_id, context, "Please complete setup first.", new_main_menu_kb(user_id))
        return
    ...
    total = len(a["targets"])
    await edit_banner_strict(user_id, context, ADS_PROGRESS_FMT.format(sent=0, total=total), new_main_menu_kb(user_id))
...
async def ads_worker(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    client = get_final_client(user_id)
    if client is None:
        await edit_banner_strict(user_id, context, "Login required to send ads.", new_main_menu_kb(user_id))
        return

    await client.connect()
    try:
        if not await client.is_user_authorized():
            await edit_banner_strict(user_id, context, "Session expired. Please login again.", new_main_menu_kb(user_id))
            return
        ...
        while True:
            sent = 0
            total = len(targets)
            await edit_banner_strict(user_id, context, ADS_PROGRESS_FMT.format(sent=sent, total=total), new_main_menu_kb(user_id))
            ...
            await edit_banner_strict(user_id, context, ADS_WAITING_FMT.format(total=total, wait=round_delay), new_main_menu_kb(user_id))
