...
if not BOT_TOKEN or not CHANNEL_1 or not CHANNEL_2:
    sys.exit("❌ .env must have BOT_TOKEN, CHANNEL_1, CHANNEL_2")
...
def verify_kb() -> InlineKeyboardMarkup:
    chan1_url = _build_tg_url_from_env(CHANNEL_1)
    chan2_url = _build_tg_url_from_env(CHANNEL_2)
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📣 Channel", url=chan1_url)],
        [InlineKeyboardButton("👥 Group", url=chan2_url)],
...
# ---------- Membership ----------
async def is_member_of(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat: str) -> bool:
    try:
        cm = await context.bot.get_chat_member(chat_id=chat, user_id=user_id)
...
async def send_log_to_user(user_id: int, log_text: str, parse_mode: Optional[str] = "HTML"):
    ...
    try:
        ...
        await logger_bot.send_message(chat_id=user_id, text=log_text, parse_mode=parse_mode, disable_web_page_preview=True)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u.get("verified"):
        try:
            sent = await context.bot.send_photo(chat_id=chat_id, photo=BANNER_URL, caption=WELCOME_CAPTION, reply_markup=verify_kb())
            set_last_msg(u, chat_id, sent.message_id, True)
        except Exception:
            sent = await context.bot.send_message(chat_id=chat_id, text=WELCOME_CAPTION, reply_markup=verify_kb())
            set_last_msg(u, chat_id, sent.message_id, False)
        save_user(user_id)
        return
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verify
    if data == "verify":
        ok1 = await is_member_of(context, user_id, CHANNEL_1)
        ok2 = await is_member_of(context, user_id, CHANNEL_2)
        ...
        if ok1 and ok2:
            ...
            return await q.answer("Verified!")
        else:
            return await q.answer("Please join both channel and group first then verify button tap.", show_alert=True)
    ...
    if data == "targets_all":
        ...
        u["ad_setup"]["targets"] = [{"display_id": g["display_id"]} for g in u["group_picker"]["groups"]]
        u["step"] = STEP_ASK_ROUND_DELAY
        save_user(user_id)
...
async def collect_user_groups(user_id: int) -> bool:
    ...
    try:
        ...
        # Fetch ALL forum topics in PARALLEL - SUPER FAST!
        if forum_fetch_tasks:
            print(f"⚡ Fetching {len(forum_fetch_tasks)} forum topics in parallel...")
            ...
            for result in topic_results:
                if isinstance(result, tuple) and len(result) == 2:
                    ...
                    topics_by_group[group_id] = topics
            
            # Add topics to their parent groups and create topic entries
            for group_entry in groups:
                if group_entry["is_forum"]:
                    disp_id = group_entry["display_id"]
                    topics = topics_by_group.get(disp_id, [])
                    
                    if topics:
                        # Store topics in group entry
                        group_entry["topics"] = [{
                            "topic_id": t["topic_id"],
                            "title": t["title"],
...
async def ads_worker(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async def send_saved_copy(dst, msg_id: int, topic_id=None):
            ...
            try:
                ...
                if msg:
                    ...
                    if topic_id:
                        await client.send_message(dst, msg.text or msg.caption or "", file=msg.media, reply_to=topic_id)
                    else:
                        await client.send_message(dst, msg.text or msg.caption or "", file=msg.media)
                else:
                ...
                try:
                    if topic_id:
                        await client.forward_messages(dst, msg_id, saved_from_peer, drop_author=True, reply_to=topic_id)
                    else:
                        await client.forward_messages(dst, msg_id, saved_from_peer, drop_author=True)
                except Exception:
                    # Last resort: just forward
                    await client.forward_messages(dst, msg_id, saved_from_peer)
        ...
        async def send_forward_with_tag(dst, msg_id: int, topic_id=None):
            ...
            try:
                if topic_id:
                    await client.forward_messages(dst, msg_id, saved_from_peer, reply_to=topic_id)
                else:
                    await client.forward_messages(dst, msg_id, saved_from_peer)
            except Exception as e:
        ...
        async def send_custom(dst, topic_id=None):
            if media_path:
                ...
                await client.send_message(dst, message_text or "", reply_to=topic_id)
                ...
                try:
                    ...
                    if saved_msg_id:
                        if saved_as_copy is False:
                            ...
                            try:
                                await send_forward_with_tag(dst, saved_msg_id, topic_id)
                                ok = True
                            except (terr.ChatForwardsRestrictedError, terr.MessageIdInvalidError, Exception) as fwd_err:
                                # Forwarding failed, try fallback custom message
                                fallback_msg = a.get("fallback_message")
                                if fallback_msg:
                                    try:
                                        await client.send_message(dst, fallback_msg, reply_to=topic_id)
                                        ok = True
                                        error_msg = "⚠️ Used fallback (forward blocked)"
                                    except Exception as fb_err:
                                        error_msg = f"❌ Forward & fallback failed: {str(fb_err)[:30]}"
                                else:
                                    raise fwd_err  # No fallback, re-raise original error
                        else:
                            await send_saved_copy(dst, saved_msg_id, topic_id)
                            ok = True
                    else:
                        await send_custom(dst, topic_id)
                        ok = True
