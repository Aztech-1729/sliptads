...
def user_file(uid: int) -> Path:
    return DATA_DIR / f"user_{uid}.json"

def list_user_ids() -> List[int]:
    ids = []
    for p in DATA_DIR.glob("user_*.json"):
        try:
            uid = int(p.stem.split("_", 1)[1])
            ids.append(uid)
        except Exception:
            pass
    return ids
...
def save_user(uid: int, data: Dict[str, Any]):
    user_file(uid).write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        async def send_to(uid: int):
            ...
            try:
                if update.message.photo:
                    ...
                    path = DATA_DIR / f"bc_photo_{secrets.token_hex(4)}.jpg"
                    ...
                    path = DATA_DIR / f"bc_video_{secrets.token_hex(4)}.mp4"
                    await file.download_to_drive(str(path))
                    with open(path, "rb") as f:
                    ...
                    path = DATA_DIR / f"bc_gif_{secrets.token_hex(4)}.mp4"
                    await file.download_to_drive(str(path))
                    with open(path, "rb") as f:
